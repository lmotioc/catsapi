...
By default, it doesn't return a great deal, but if you visit `http://localhost:3001/cats` -- which will automatically issue a GET request to our running API server -- you'll see a simple JSON object populated with some cat data.
...
2. Add a route file that will access this data into `./routes/[your route file].js` -- hint: use the `./routes/cats.js` as a starting point
